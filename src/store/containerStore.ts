...
export const useContainerStore = create<ContainerState>((set, get) => ({
  ...
  loadContainers: async () => {
    ...
    try {
      ...
      set({
        error: err instanceof Error ? err.message : 'Error al cargar contenedores',
        isLoading: false,
      });
    }
  },
  ...
  loadContainerById: async (id: string) => {
    ...
    try {
      ...
      set({
        error: err instanceof Error ? err.message : 'Error al cargar contenedor',
        isLoading: false,
      });
    }
  },
  ...
  createContainer: async (data: CreateContainerInput) => {
    ...
    try {
      ...
      set({
        error: err instanceof Error ? err.message : 'Error al crear contenedor',
        isLoading: false,
      });
      throw err;
    }
  ...
  updateContainer: async (id: string, data: UpdateContainerInput) => {
    ...
    try {
      ...
      // Refresh the updated container in the list
      const updated = await ContainerRepository.findById(id);
      if (updated) {
        set((state) => ({
          containers: state.containers.map((c) => (c.id === id ? updated : c)),
          selectedContainer:
            state.selectedContainer?.id === id ? updated : state.selectedContainer,
        }));
      }
    } catch (err) {
      set({
        error: err instanceof Error ? err.message : 'Error al actualizar contenedor',
      });
      throw err;
    }
  },
  ...
  deleteContainer: async (id: string) => {
    ...
    try {
      ...
      set({
        error: err instanceof Error ? err.message : 'Error al eliminar contenedor',
      });
      throw err;
    }
  },
